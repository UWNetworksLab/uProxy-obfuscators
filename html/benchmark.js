        function WriteFteTable() {
            for (var i = 0; i < test_languages.length; i++) {
              ...
              retval += '<td class="testInputLanguage">('+test_languages[i]['plaintext_regex']+', '+test_languages[i]['plaintext_max_len']+')</td>';
              retval += '<td class="testInputLaguage">('+test_languages[i]['ciphertext_regex']+', '+test_languages[i]['ciphertext_max_len']+')</td>';
  ...
  var ciphertext_dfa = regex2dfa(test_languages[i]['ciphertext_regex']);
              retval = do_fte_benchmark(plaintext_dfa, test_languages[i]['plaintext_max_len'],
              ...
              if (success) {
                 ...
                 var toUpdate = document.getElementById('fte_row'+i).getElementsByClassName('testCost')[0];
...
var test_lengths = [128, 256, 512, 1024, 2048, 4096, 8192];
        ...
        function FillRabbitTable(i) {
              ...
              if (success) {
                 ...
                 var toUpdate = document.getElementById('rabbit_row'+i).getElementsByClassName('testCost')[0];
                 toUpdate.innerHTML = elapsed;
