...
private:
    ...
    // our rank_unrank start state
    uint32_t _start_state;

    // the number of states in our rank_unrank
    uint32_t _num_states;

    // the number of symbols in our rank_unrank alphabet
    uint32_t _num_symbols;

    // the symbols of our rank_unrank alphabet
...
public:
    // The constructor of our rank/urank rank_unrank class
    rank_unrank( const std::string, const uint32_t );

    // our unrank function an int -> str mapping
    // given an integer i, return the ith lexicographically ordered string in
    // the language accepted by the rank_unrank
    ...
    // rank_unrank that are at least length n and no greater than length m
    mpz_class getNumWordsInLanguage( const uint32_t, const uint32_t );
};

// given a perl-compatiable regular-expression
// returns a (non-minimized) ATT FST formatted rank_unrank
